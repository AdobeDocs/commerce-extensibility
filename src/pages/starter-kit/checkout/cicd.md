...
To learn more about CI/CD for App builder, refer to [CI/CD for App Builder Applications](https://developer.adobe.com/app-builder/docs/guides/app_builder_guides/deployment/cicd-for-app-builder-apps).
...
The [deploy_prod.yml](https://github.com/adobe/commerce-checkout-starter-kit/blob/main/.github/workflows-samples/deploy_prod.yml) workflow triggers on a release event ([Github Events that trigger Workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#release)), which is when a release is marked as released.  It also defines a single job called `deploy`, which performs the following steps sequentially to deploy the application to production.

1. **Environment Setup** - Performs the following setup steps:
   - [Matrix Strategy](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/run-job-variations#about-matrix-strategies): The job runs on Ubuntu (ubuntu-latest) using Node.js version 20. It only allows one parallel execution (`max-parallel: 1`) and executes the following steps:
...
1. **Authentication** - Authenticates with `Adobe I/O` using secure credentials stored as secrets (for example, `CLIENTID_PROD` and `CLIENTSECRET_PROD`). Refer to [Secrets in Github Actions](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-secrets) to configure variables as secrets.
...
The [deploy_stage.yml](https://github.com/adobe/commerce-checkout-starter-kit/blob/main/.github/workflows-samples/deploy_stage.yml) workflow is the same as the [Production deployment example](#production-deployment-example), with the following key differences:
