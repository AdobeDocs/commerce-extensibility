...
The `path` parameter specifies where to redirect an action. The Admin UI SDK provides the selected product IDs in a [`sharedContext`](https://developer.adobe.com/commerce/extensibility/admin-ui-sdk/extension-points/#shared-contexts) when a merchant selects a mass action. Your implementation must read the selected items from the `sharedContext`.

<!--- Added a hard link above and in the parameters table because the relative links weren't resolving correctly. -->
...
| `displayIframe` | boolean | No | Indicates whether an iFrame will be displayed at the relative path. The default value is `true`. [Mass actions without iFrames](https://developer.adobe.com/commerce/extensibility/admin-ui-sdk/extension-points/#mass-actions-without-iframes) provides additional details. |
