---
title: Skills, prompts, and commands for Commerce development MCPs
...
When you select the **AEM Boilerplate Commerce** starter kit during [setup](dropins-mcp-server.md), a separate set of skills is installed for storefront development. These skills route drop-in data queries (slots, events, containers, and API functions) to Dropins AI Tools as the primary reference, and fallback to the TypeScript definitions in `node_modules/@dropins/` when the server is unavailable.
