...
Processor classes run during the `event_data_batch_send` cron job. Their output is sent directly to App Builder as part of the event payload. Therefore, custom fields generated by processor classes are not added to the `event_data` table in Adobe Commerce.
